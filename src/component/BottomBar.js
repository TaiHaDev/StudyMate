...
const BottomBar = ({ onRemoveBackgroundHandler }) => {
  const [musicThumbnail, setMusicThumbnail] = useState(
    "https://i.ytimg.com/vi/JxJVQvuVlnU/hq720.jpg?sqp=-oaymwEcCOgCEMoBSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLCKwkBIDkwqQtdnJ1yVCFB5FPYeag"
  );
    ...
    <div className="inline-flex space-x-4 rounded-full bg-slate-200 dark:bg-slate-800 dark:text-white px-2 py-1 shadow-lg">
      <div className="hover:bg-slate-600  rounded-full">
        ...
        <img
          src={musicThumbnail}
          alt="music thumbnail"
          className="w-8 h-8 object-cover rounded-full spin-slow object-center"
          onClick={onClickMusicHandler}
        />
      </div>

      <div
        className="hover:bg-slate-600 px-2 py-1 rounded-full"
        onClick={onRemoveBackgroundHandler}
      >
        <svg
